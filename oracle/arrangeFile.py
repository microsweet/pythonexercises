...
def query_data():
    ...
    #错误信息写入excel表格
    write_excel_xlsx('/home/microsweet/arrangeFile/errorExcel.xls', 'sheet1',
                     errorRows)
    print('end')
...
def arrangeFile(taskRow, feedbackRow, fileRow, conn, curs):
    ...
    targetFilePath = taskRow[3] + '企业土地核查情况汇总/' \
                   ...
                   + str(feedbackRow[1]) + '/' \
                   + fileTypeName
    sourceFilePath = fileRow[1].replace('D:/landtax/uploadFile/basicInfo/',
                                        '/home/microsweet/arrangeFile/read/')
    #查看是否有目标目录
